...
let
    ...
    folders = lib.pipe folders [
      ...
      (lib.mapAttrs (
        ...
        let
          ...
          untrustedTargetNames = lib.filter (name: devices.${name} ? "encrypted") (lib.attrNames devices);
  ...
  vpnFor =
    ...
    let
      ...
      other = lib.mapAttrsToList (_: v: v) (
        if host ? "server" && host.server then # get all other devices
    ...
    in
    {
      ...
      text = ''
        ...
        ${lib.concatMapStringsSep "\n" (peer: ''
          ...
          ${
            if peer.vpn ? "server" && peer.vpn.server then
              ''
